...
from ..db import ClauseRepository, ContractRepository, FindingRepository, get_cosmos_client
from ..models.clause import Clause
from ..models.contract import Contract
from ..models.finding import LeakageFinding, Severity
from ..utils.brand_constants import BrandColors, Layout, ReportConfig, Typography
...
class ReportService:
    ...
    def generate_pdf_report(self, contract_id: str, include_clauses: bool = False) -> bytes:
        """
    ...
    def _get_report_data(self, contract_id: str) -> tuple[Contract, List[Clause], List[LeakageFinding]]:
    ...
    def _build_cover_page(self, contract: Contract, findings: List[LeakageFinding]) -> List:
        ...
        cover_data = [[Paragraph(ReportConfig.REPORT_TITLE, title_style)]]
    ...
    def _build_executive_summary(self, contract: Contract, findings: List[LeakageFinding]) -> List:
    ...
    def _build_findings_section(self, findings: List[LeakageFinding], styles) -> List:
    ...
    def _build_clauses_section(self, clauses: List[Clause], styles) -> List:
        ...
        for i, clause in enumerate(clauses[:50], 1):  # Limit to first 50
            ...
            story.append(
                Paragraph(
                    clause.normalized_summary or clause.original_text[:500],
                    styles["Normal"],
                )
            )
    ...
    def _create_summary_sheet(self, wb: openpyxl.Workbook, contract: Contract, findings: List[LeakageFinding]):
    ...
    def _create_findings_sheet(self, wb: openpyxl.Workbook, findings: List[LeakageFinding]):
    ...
    def _create_clauses_sheet(self, wb: openpyxl.Workbook, clauses: List[Clause]):
        ...
        for row, clause in enumerate(clauses, 2):
            ...
            ws.cell(row=row, column=5).value = (
                f"{clause.extraction_confidence * 100:.0f}%" if clause.extraction_confidence else "N/A"
            )
